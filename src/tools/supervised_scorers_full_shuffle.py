...
def get_relative_scores_supervised(estimator, calibration_faces_observed, target_face, calibration_eeg_observed, observation_eeg_observed, distances_hypothesis):
    ...
    shuffled_estimator = clone(estimator)
    ...
    shuffled_calibration_distances = rng.permutation(calibration_distances)
    shuffled_calibration_scaler = StandardScaler()
    shuffled_scaled_calibration_distances = shuffled_calibration_scaler.fit_transform(np.array(shuffled_calibration_distances).reshape(-1, 1)).flatten().tolist()

    shuffled_estimator.fit(calibration_eeg_observed, shuffled_scaled_calibration_distances)
    
    shuffled_eeg = observation_eeg_observed[rng.permutation(len(observation_eeg_observed))]
    scaled_shuffled_distance = shuffled_estimator.predict(shuffled_eeg)
    shuffled_pred_distance = shuffled_calibration_scaler.inverse_transform(np.array(scaled_shuffled_distance).reshape(-1, 1)).flatten().tolist()
    ...
    for scoring_method in scorings:
        ...
        if scoring_method == 'neg_root_mean_squared_error':
            ...
            # Shuffled scores
            shuffled_score = -np.sqrt(mean_squared_error(distances_hypothesis, shuffled_pred_distance))
    ...
    # results['pred_distance'] = pred_distance
    # results['shuffled_pred_distance'] = shuffled_pred_distance
